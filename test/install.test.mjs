...
test('install creates substituted pointer and executable husky hooks', async () => {
  ...
  try {
    ...
    assert.match(await readFile(join(host, '.mcp', 'figma-mcp-go.example.json'), 'utf8'), /figma-mcp-go/)
    assert.match(await readFile(join(host, '.mcp', 'figma-mcp-go.md'), 'utf8'), /Figma Desktop/)
    ...
    assert.match(doctor.stdout, /projectmem MCP example avoids local absolute paths/)
