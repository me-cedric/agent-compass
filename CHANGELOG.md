...
- **`openspec-guard` reads the chain a project actually declares.** The `chain`
  ...
  absent, so a host on its own schema was judged by a chain it does not use. A
  schema that writes no `design.md` and keeps `tasks.md` beside each capability
  spec had both reported missing on every complete change — an error it could
  never clear. The offline fallback now reads
