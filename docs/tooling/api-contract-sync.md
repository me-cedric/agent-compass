...
| ---------------- | -------------------------------------- | ----------------------------------------------------- |
| OpenAPI / Scalar | controller + DTO decorators            | `@ZodResponse`, `@ApiOperation`, `@ApiResponse`, `@ApiQuery`, `@ApiParam`, `@ApiProperty`, `@ApiTags`. Register new modules in the Swagger bootstrap. |
...
| Shared types     | `packages/**`, `libs/**`, API clients  | Request/response contracts exported to consumers.     |
...
- [ ] `@ZodResponse` or equivalent response contract updated
- [ ] DTO `@ApiProperty` metadata updated
- [ ] Shared request/response types updated and consumers identified
