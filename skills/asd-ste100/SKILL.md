...
Agent Compass normalized the frontmatter `name` from
`Simplified Technical English (ASD-STE100)` to the slug `asd-ste100`, because
agent harnesses key a skill by a slug that matches its folder. It also added
the compass frontmatter fields (`risk_level`, `writes_files`,
`requires_tools`). The upstream description, version, and rule text remain.
