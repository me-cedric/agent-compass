...
name: asd-ste100
description: >
  Simplified Technical English (ASD-STE100). Rewrites ambiguous English into
  ASD-STE100 style — one meaning per word, active voice, simple tense, short
  sentences. Use for durable text that is read later without you: commit
  messages, pull request descriptions, docs, specs, ADRs, error messages, tool
  descriptions, and instructions handed to another agent. Also use when the
  user says "simplify this", "STE100 rewrite", or reports that agent output is
  hard to parse.
risk_level: low
writes_files: false
requires_tools: []
...
Vendored from [`danyuchn/asd-ste100-skill`](https://github.com/danyuchn/asd-ste100-skill)
at commit `8564f8985f15104c2184f90531bfd1bbb25f3d5b`, MIT License,
© 2026 Dustin Yuchen Teng. Full terms in `LICENSE` next to this file.

Two changes were made on import. The frontmatter `name` was normalized from
`Simplified Technical English (ASD-STE100)` to the slug `asd-ste100`, because
agent harnesses key a skill by a slug that matches its folder. The full title
moved into `description`, and the compass frontmatter fields (`risk_level`,
`writes_files`, `requires_tools`) were added. No rule text was changed.

See [`docs/guidelines/style-contract.md`](../../docs/guidelines/style-contract.md)
for how this skill combines with `caveman`, `ponytail`, and `i-have-adhd`.
