---
name: image-to-code
description: Elite website image-to-code skill for a harness that can generate images. For visually important web tasks, it must first generate the design image(s) itself, deeply analyze them, then implement the website to match them as closely as possible. With image generation, it must prefer large, readable, section-specific images instead of tiny compressed boards, generate fresh standalone images for sections or detail views instead of cropping old ones, avoid lazy under-generation, avoid cards-inside-cards-inside-cards UI, and keep the hero clean, spacious, readable, and visible on a small laptop.
risk_level: medium
writes_files: true
requires_tools: []
---
...
- With image generation, bias strongly toward larger, more analyzable section images.
...
## 4. SECTION IMAGE RULE FOR AN IMAGE-CAPABLE HARNESS

With image generation, do not compress too many website sections into one single image if that would make the text, spacing, buttons, or layout details too small to analyze properly.

With image generation, prefer separate large images per section.

Default rule with image generation:
...
Without image generation, this skill may still allow more compact multi-section composition when appropriate.
With image generation, prioritize section clarity and extraction accuracy.
...
With image generation, treat each section as its own analyzable unit.
...
With image generation, these should usually become section-by-section images, not one compressed sheet.
...
4. with image generation, prefer one large image per section
...
With image generation, if the user wants multiple sections, prefer separate large section images instead of one compressed multi-section board, so text, spacing, typography, buttons, and colors can be extracted properly.
...
`skills/image-to-code-skill`. The licence is MIT, © 2026 Leonxlnx. The `LICENSE` file is
next to this file.

Changes made on import: the folder is renamed to the frontmatter `name`, the
compass frontmatter fields (`risk_level`, `writes_files`, `requires_tools`) are
added, and provider-specific wording is replaced by the capability it means (a
harness that can generate images). No design rule was changed.
