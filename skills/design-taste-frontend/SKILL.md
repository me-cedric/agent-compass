...
2. **Real web images second.** When no gen tool is available, use real photography sources. Acceptable defaults:
   * a labelled `<!-- TODO: image -->` slot for placeholder photography
...
* **NO broken Unsplash links.** Use a labelled `<!-- TODO: image -->` slot, or generated photo placeholders, or actual assets.
...
* **NO photo-credit captions as decoration.** Strings like `Field study no. 12 · Ines Caetano`, `Plate 03 · House archive`, `Frame XII · 35mm` under stock/placeholder images are pretentious. Photo credit is allowed ONLY when there is a real photographer being credited for a real photo (with permission). Otherwise: skip the caption or use a one-line functional caption ("The 6-quart, in Sage.").
...
- [ ] **Real images used** (gen-tool first, then placeholder-seed, then explicit placeholder slots) - NO div-based fake screenshots, NO hand-rolled decorative SVGs, NO pure-text minimalism?
...
`skills/taste-skill`. The licence is MIT, © 2026 Leonxlnx. The `LICENSE` file is
next to this file.

Three changes were made on import:

- The folder is renamed to the frontmatter `name`. An agent harness keys a skill
  by a slug that is equal to its folder name, and upstream disagrees with its own
  frontmatter in most folders.
- Each recommendation of a remote placeholder-image host is removed. A skill must
  not teach an agent to fetch an image from an unvetted remote host.
- The compass frontmatter fields (`risk_level`, `writes_files`,
  `requires_tools`) are added. Upstream declares none of them, and every skill
  here rewrites frontend source files.
