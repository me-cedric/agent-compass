// upstream-skills.mjs — the install-time adapter for the tracked operational
// corpus. Agent Compass keeps no copy of those skills, so this is what corrects
// them on the way into a host: the safety gate, the compass frontmatter, the
// eight argv-secret narrowings, and the provenance block.
// Used by scripts/external-skills.mjs. See docs/tooling/operational-skills.md.
...
// Local overrides run on the way out of the upstream tree, at install time, so
// every install of a given pin produces the same corrected text and no
// uncorrected copy is ever stored. Record why each
